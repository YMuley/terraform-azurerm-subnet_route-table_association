...
resource "azurerm_subnet_route_table_association" "subnet_route_table_association" {
  ...
  subnet_id      = each.value.subnet_name == data.azurerm_subnet.subnet.name ? data.azurerm_subnet.subnet.id : "" #var.subnet_output[format("%s/%s", each.value.virtual_network_name, each.value.subnet_name)].id
