...
resource "azurerm_subnet_route_table_association" "subnet_route_table_association" {
  ...
  subnet_id      = flatten([for subnet in data.azurerm_subnet.subnet : subnet.id if (each.value.subnet_name == subnet.name) == true]) #var.subnet_output[format("%s/%s", each.value.virtual_network_name, each.value.subnet_name)].id
